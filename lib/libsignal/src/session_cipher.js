...
import { RECEIVING, SENDING } from './chain_type.js';
...
import { deriveSecrets, encrypt as _encrypt, calculateMAC, verifyMAC, decrypt } from './crypto.js';
import { generateKeyPair, calculateAgreement } from './curve.js';
import { SessionError, UntrustedIdentityKeyError, MessageCounterError } from './errors.js';
import { WhisperMessage, PreKeyWhisperMessage } from './protobufs.js';
...
class SessionCipher {
    ...
    async encrypt(data) {
        ...
        return await this.queueJob(async () => {
            ...
            if (chain.chainType === RECEIVING) {
                throw new Error("Tried to encrypt on a receiving chain");
            }
            this.fillMessageKeys(chain, chain.chainKey.counter + 1);
            const keys = deriveSecrets(chain.messageKeys[chain.chainKey.counter],
            ...
            msg.ciphertext = _encrypt(keys[0], data, keys[2].slice(0, 16));
            ...
            const mac = calculateMAC(keys[1], macInput);
    ...
    async doDecryptWhisperMessage(messageBuffer, session) {
        ...
        const keys = deriveSecrets(messageKey, Buffer.alloc(32),
        ...
        verifyMAC(macInput, keys[1], messageBuffer.slice(-8), 8);
        const plaintext = decrypt(keys[0], message.ciphertext, keys[2].slice(0, 16));
    ...
    fillMessageKeys(chain, counter) {
        ...
        chain.messageKeys[chain.chainKey.counter + 1] = calculateMAC(key, Buffer.from([1]));
        chain.chainKey.key = calculateMAC(key, Buffer.from([2]));
    ...
    maybeStepRatchet(session, remoteKey, previousCounter) {
        ...
        ratchet.ephemeralKeyPair = generateKeyPair();
    ...
    calculateRatchet(session, remoteKey, sending) {
        ...
        const masterKey = deriveSecrets(sharedSecret, ratchet.rootKey,
        ...
        session.addChain(chainKey, {
            ...
            chainType: sending ? SENDING : RECEIVING
        });
